...
class IDetect_fms(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        """
        super(IDetect_fms, self).__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        # 如果num_cls和yaml中的配置不一样，优先使用传入nc
        if nc and nc != self.yaml['nc']:
            logger.info(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
            self.yaml['nc'] = nc  # override yaml value
        if anchors: # 默认是None(实际的anchors在self.yaml中定义了)
    ...
    def forward(self, x, augment=False, profile=False):
        if augment: # 默认为False，不执行
            ...
            for si, fi in zip(s, f):
                xi = scale_img(x.flip(fi) if fi else x, si, gs=int(self.stride.max())) # 按照s中的比例进行放缩（并通过pad/cut保证是gridsize的整数倍）
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if not hasattr(self, 'traced'):  # 默认存在，且为False
                self.traced=False

            if self.traced:  # 默认为False
                if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint):
                    break

            if profile:  # 训练时默认False
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    # print("anchors from cfg", d["anchors"])
    # [[10, 13, 16, 30, 33, 23], [30, 61, 62, 45, 59, 119], [116, 90, 156, 198, 373, 326]]
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors # yaml中有3个anchor列表，对应不同scale的layer
    no = na * (nc + 5)  # number of outputs = anchors * (classes + 5) # 这里的5是什么？[cls xyhw]么?

    layers, save, c2 = [], [], ch[-1]  # layers, savelist, ch out
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        m = eval(m) if isinstance(m, str) else m  # eval strings # 注意这里的m是layer类型，大部分定义在models.common中
        for j, a in enumerate(args):
            try:
                args[j] = eval(a) if isinstance(a, str) else a  # eval strings # 主要是激活函数
            except:
                pass
