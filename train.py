...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Directories 
    # save_dir 为runs/train/TRAIN_PROJ_NAME/
    # 权重保存在 runs/train/TRAIN_PROJ_NAME/weights目录下
    ...
    # Configure
    plots = not opt.evolve  # create plots # evolve默认False, plots为Ture
    cuda = device.type != 'cpu'
    # ============= init seed ===========
    # 初始化python(random),numpy和torch的cpu/GPU模块的RNG(随机数发生器)，一定程度上保证结果复现的一致性.
    # 这里还引入了rank(进程ID, 多进程间通信和进程优先级的表征)，一定程度上维持不同epoch情况下的，同一数据同样参数的结果一致性。
    init_seeds(2 + rank) 
    # ===================================
    with open(opt.data) as f: # 加载fms.yaml（记录数据集路径和cls类型）
        data_dict = yaml.load(f, Loader=yaml.SafeLoader)  # data dict
    # 判断是否是coco数据集
    is_coco = opt.data.endswith('coco.yaml')

    # ================ logging的配置，只有master节点拥有 =====================
    # Logging- Doing this before checking the dataset. Might update data_dict
    loggers = {'wandb': None}  # loggers dict
    if rank in [-1, 0]: # -1表示cpu, 0表示单GPU或多GPU的master节点进程 
        opt.hyp = hyp  # add hyperparameters # 将hyp的data对象赋值给opy.hyp(后者原先是路径)
    ...
    if pretrained:
        with torch_distributed_zero_first(rank): # 自定义功能的上下文管理器,这里主要是应对多GPU训练时，等待主节点master执行(尝试)下载任务，其他进程全部阻塞等待
    ...
    model.class_weights = labels_to_class_weights(dataset.labels, nc).to(device) * nc  # attach class weights # 统计样本的类别个数，计算对应类别的权重
    model.names = names # 当前训练的别名。例如：yolov7-fms-singlecls

    # Start training
    t0 = time.time()
    # 热启动配置：这里的
    nw = max(round(hyp['warmup_epochs'] * nb), 1000)  # number of warmup iterations, max(3 epochs, 1k iterations) # tiny默认的warmup_epochs是3.
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= nw:
                ...
                accumulate = max(1, np.interp(ni, xi, [1, nbs / total_batch_size]).round()) # np.interp是线性插值，在xi和[1, nbs/total_batch_size]确认的直线上，确定x=ni时的坐标。
