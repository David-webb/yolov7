...
def detect(save_img=False):
    ...
    stride = int(model.stride.max())  # model stride # 这是个啥？根据下面“验证imgsz是stride的整数倍”的操作，猜测可能是下采样的倍数
    print("stride的数值是:", stride) # yolov7-tiny是32
    ...
    if webcam:
        view_img = check_imshow() # 检查当前环境是否可以显示检测结果
        cudnn.benchmark = True  # set True to speed up constant image size inference
        dataset = LoadStreams(source, img_size=imgsz, stride=stride) # 起线程单独的拉取所有的视频帧并缓存在 # 每4帧读取一帧
    else:
    ...
    # Run inference
    if device.type != 'cpu':
        model(torch.zeros(1, 3, imgsz, imgsz).to(device).type_as(next(model.parameters())))  # run once# 这里为什么要进行一次推理
    ...
    print(f'Done. ({time.time() - t0:.3f}s)', ccc)
